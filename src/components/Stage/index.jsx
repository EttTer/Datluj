  ...
  const handleModalClose = () => {
    ...
    if (timeLeft === 0) {
      const playerName = prompt(
        "Prosím zadejte jméno hráče do listiny výsledků:"
      );

      if (playerName && playerName.trim() !== "") {
        setPlayerName(playerName);
        setIsGameStarted(false);
        let message = `Hra skončila, napsal(a) jsi ${wordsWritten} slov a udělal(a)  ${mistakes} chyb.\n\n`;
        if (mistakeWords.length > 0) {
          message += "Slovo s chybou:\n";
          const uniqueMistakeWords = [...new Set(mistakeWords)];
          uniqueMistakeWords.forEach((word) => {
            const mistakesInWord = mistakeWords.filter(
              (w) => w === word
            ).length;
            message += `Slovo ${word} - ${mistakesInWord} Chyby\n`;
          });
        }
        const result = {
  ...
  return (
    <>
      <div className="Stage_page_container">
        ...
        <button className="Start_page_button">
          <Link className to="/starter">
            Hra bez časovéhu limitu
          </Link>
        </button>
      ...
      <div className="stage">
        ...
        >
          ...
          <Modal.Body>
            <p>
              Napsal(a) jsi {wordsWritten} slov a udělal(a) jsi {mistakes} chyb.
            </p>
